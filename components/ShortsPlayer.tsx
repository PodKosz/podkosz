...
/**
 * Shorts z boiska. Domyślnie stoi miniatura z pomarańczowym trójkątem w szkle —
 * film wczytuje się dopiero po kliknięciu, więc nic nie startuje samo i nie zjada danych.
 */
export function ShortsPlayer({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[330px]">
      <div className="glass relative overflow-hidden rounded-[28px] p-1.5">
        <div className="relative aspect-[9/16] overflow-hidden rounded-[24px] bg-black">
          {playing ? (
            ...
            >
              ...
              <span className="absolute left-1/2 top-1/2 grid h-[84px] w-[84px] -translate-x-1/2 -translate-y-1/2 place-items-center">
                <span
                  className="absolute inset-[-26px] rounded-full opacity-80 blur-md transition group-hover:opacity-100"
                  style={{
                    background:
                ...
                <span className="relative grid h-full w-full place-items-center rounded-full border border-white/30 bg-white/10 shadow-[0_10px_30px_-8px_rgba(0,0,0,.8)] backdrop-blur-md transition duration-300 group-hover:scale-[1.06] group-hover:border-white/45 group-hover:bg-white/16">
                  <PlayIcon className="ml-1.5 h-9 w-9 text-flame drop-shadow-[0_1px_6px_rgba(255,77,10,.55)]" />
                </span>
              </span>

              <span className="absolute inset-x-4 bottom-4 text-left text-[11px] uppercase tracking-[0.16em] text-white/80">
                obejrzyj boisko na filmie
              </span>
            </button>
          )}
        </div>
